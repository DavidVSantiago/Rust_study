...
/** Retorna um numero aleatório no intervalo
 * use Intervalo::Total para intervalor total do u32
 * use Intervalo::Entre(u32,u32) para intervalo [min,max]
 */
pub fn rand(i:Intervalo) -> u32 {
    match i {
        Intervalo::Total => rand_total(), // retorna o intervalo total
        Intervalo::Entre(a, b) => rand_entre(a, b),
    }
}
...
/** gera uma série de caracteres ASCII de comprimento tam */
pub fn rand_senha(tam: usize, t: TipoSenha) -> String{
    ...
    for item in senha.iter_mut(){
        ...
        let indice:usize = rand(Intervalo::Entre(0, c_tam-1)) as usize;
...
/** Respresenta o intervalo dos números aleatórios */
pub enum Intervalo {
    Total,
    Entre(u32, u32),
}
pub enum TipoSenha {
    Numeros,LetrasMaiusculas,LetrasMinusculas,Letras,NumerosLetras,Especiais,Total,
}
...
fn rand_total() -> u32
{
    let mut rng = StdRand::default(); // obtém o objeto ThreadRng
    rng.next_u32()
}

/** Retorna um número aleatório no intervalo [a,b] inclusivo*/
fn rand_entre(mut a: u32, mut b: u32) -> u32 {
    // verificaçao de Guarda
    if a > b { std::mem::swap(&mut a, &mut b); }
    let mut rng = StdRand::default(); // obtém o objeto ThreadRng
    rng.next_range(a..b+1)
}
